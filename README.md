...
Также есть метод sumNumbers, который вычисляет сумму десятичных дробей, реализующих интерфейс Calculate.
Сам Calculate это интерфейс для объектов, которые представлены десятичной дробью. Далее методы абстрактного класса Number для приведения к примитивным типам.
Просто выполняется процесс деления числителя на знаменатель (Как в дроби) исходя из типа данных.
...
Main - внутри файл main с основным меню пользователя и  интерфейсом
Fraction - внутри файл Fraction для всех действий с дробями и Calculate с intarface для 5 задания
